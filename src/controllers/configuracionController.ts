...
export async function createPago(req: Request, res: Response) {
  ...
  try {
    ...
    const monedaPrincipal = configuracion?.monedaPrincipal || "USD";
    ...
    for (const pago of pagos) {
      ...
      const tipo = pago.moneda;

      if (tipo === monedaPrincipal) {
        totalPagado += valor;
      } else if (tipo === "VES" && configuracion?.tasaVES) {
        totalPagado += valor / configuracion.tasaVES;
      } else if (tipo === "COP" && configuracion?.tasaCOP) {
        totalPagado += valor / Number(configuracion.tasaCOP);
      }
    }
